function [params, Accept_ratio, acceptRate] = mcmc_bad(isConstrained, A0, which_sample)
...
for i_rep = 2:N
    ...
    for j = 1:n
        Dprop = Dold;
        Aold(j, j) = 1;
        Dprop(j) = ((1 - 2*(rand(1) <= 0.5))*sqrt(Dprop(j)) + ...
            randn(1) * sdProp).^2;
        ...
        for i = 1:m
            Aprop = Aold;
            if ((~isConstrained(i, j))&&(which_sample(i , j))&&(i ~= j))
