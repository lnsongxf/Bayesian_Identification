function [params,  Accept_ratio, acceptRate] = mcmc_bad3(isConstrained, A0, which_sample, var_prior, nBurn, nDraw)
N = nBurn + nDraw;
        ...
        for i = 1:m
            ...
            if ((~isConstrained(i, j))&&(which_sample(i , j) == 1)&&(i ~= j))
                ...
                Accept_ratio(i_rep, indx(i, j)) = ...
                    exp( -0.5*Aprop(i, j).^2/var_prior(1) + ...
                    0.5*Aold(i, j).^2/var_prior(1) + ...
                nll_bad(Aold, Dold) - nll_bad(Aprop, Dold));
                isAccepted = rand(1) <= Accept_ratio(i_rep, indx(i, j));
                if isAccepted
                    nAccepted(i, j) = nAccepted(i, j) + 1;
                    Aold = Aprop;
                end
            elseif (isConstrained(i, j))
                Aprop(i, j) = Aprop(i, j) + randn(1) * sdProp;
                
                Accept_ratio(i_rep, indx(i, j)) = ...
                    exp( -0.5*Aprop(i, j).^2/var_prior(2) + ...
                    0.5*Aold(i, j).^2/var_prior(2) + ...
                nll_bad(Aold, Dold) - nll_bad(Aprop, Dold));
                isAccepted = rand(1) <= Accept_ratio(i_rep, indx(i, j));
                if isAccepted
                    nAccepted(i, j) = nAccepted(i, j) + 1;
